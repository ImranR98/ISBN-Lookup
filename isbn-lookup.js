...
async function fetchBookDataFromISBNdb(isbn, apiKey) {
  ...
  if (!isbnDbResponseData.book) {
    throw JSON.stringify(isbnDbResponseData, null, '\t')
  }
...
async function fetchBookDataFromGoogleBooks(isbn) {
  ...
  if (!googleBooksResponseData.items || googleBooksResponseData.items.length == 0) {
    throw JSON.stringify(googleBooksResponseData, null, '\t')
  }
...
async function fetchBookDataFromOpenLibrary(isbn) {
  ...
  if (!openLibraryResponseData[`ISBN:${isbn}`]) {
    throw JSON.stringify(openLibraryResponseData, null, '\t')
  }
